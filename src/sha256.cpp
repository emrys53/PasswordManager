...
static std::vector<uint32_t> padding(const std::string_view &password) {
     ...
     */
    const std::size_t mul_512 = 1 + (password_length * 8 + 64 + 1) / SHA256_CHUNK_SIZE;

    /*
     * Reserving mul_512*512 number of bits in uint32_t vector.
     */
    std::vector<uint32_t> block(mul_512 * SHA256_CHUNK_SIZE / 32);
...
static std::array<uint32_t, 8> compute_sha256(const std::vector<uint32_t> &block) {
    /*
    ...
    for (std::size_t i = 0; i < chunk; ++i) {
        ...
        for (std::size_t j = 0; j < 64; ++j) {
            uint32_t S1 = (std::rotr(hash_current.at(4), 6)) xor(std::rotr(hash_current.at(4), 11)) xor
                          (std::rotr(hash_current.at(4), 25));
            uint32_t ch = (hash_current.at(4) & hash_current.at(5)) ^((~hash_current.at(4)) & hash_current.at(6));
            uint32_t temp1 = hash_current.at(7) + S1 + ch + hash_constant.at(j) + w.at(j);
            uint32_t S0 = (std::rotr(hash_current.at(0), 2)) xor(std::rotr(hash_current.at(0), 13)) xor
...
std::array<uint32_t, 8> sha256_digest(const std::string_view &password) {
    const auto block = padding(password);
    const auto hashed_values = compute_sha256(block);
